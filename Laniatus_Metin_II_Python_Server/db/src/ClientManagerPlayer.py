...
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
            peer.EncodeHeader(HEADER_DG_ITEM_LOAD, dwHandle, sizeof(uint) + sizeof(TPlayerItem) * dwCount)
...
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
        snprintf(queryStr, sizeof(queryStr), "SELECT " + "id,name,job,voice,dir,x,y,z,map_index,exit_x,exit_y,exit_map_index,hp,mp,stamina,random_hp,random_sp,playtime," + "gold,level,level_step,st,ht,dx,iq,exp," + "stat_point,skill_point,sub_skill_point,stat_reset_count,part_base,part_hair," + "part_acce," + "skill_level,quickslot,skill_group,alignment,horse_level,horse_riding,horse_hp,horse_hp_droptime,horse_stamina," + "UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play),horse_skill_point FROM player%s WHERE id=%d", GetTablePostfix(), packet.player_id)
...
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
        peer.EncodeHeader(HEADER_DG_AFFECT_LOAD, dwHandle, sizeof(uint) + sizeof(uint))
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
...
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
    peer.EncodeHeader(HEADER_DG_AFFECT_LOAD, dwHandle, sizeof(uint) + sizeof(uint) + sizeof(TPacketAffectElement) * dwCount)
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
...
#ORIGINAL LINE: if ((iNumRows = mysql_num_rows(pRes)) == 0)
    if (iNumRows = mysql_num_rows(pRes)) == 0:
        dwCount = 0
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
        peer.EncodeHeader(HEADER_DG_QUEST_LOAD, dwHandle, sizeof(uint))
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
        peer.Encode(dwCount, sizeof(uint))
        return
...
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
    peer.EncodeHeader(HEADER_DG_QUEST_LOAD, dwHandle, sizeof(uint) + sizeof(TQuestTable) * dwCount)
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
    peer.Encode(dwCount, sizeof(uint))
# Laniatus Games Studio Inc. |  TODO TASK: There is no Python equivalent to 'sizeof':
